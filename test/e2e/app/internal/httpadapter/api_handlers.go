...
func (h *APIHandler) messageHandler() func(g *gin.Context) {
	return func(g *gin.Context) {
		...
		ctx := requestCtx
		if len(messageIn.TraceData) > 0 {
			ctx = monitoring.ExtractTraceData(ctx, messageIn.TraceData)
		}
		...
		status, errs = h.sendMessage(ctx, &messageIn)
		...
		logger.Error().Err(errs).Msg("failed to send message")
		g.AbortWithStatusJSON(status, errs.Error())
	}
}

func (h *APIHandler) sendInBulk() func(g *gin.Context) {
	return func(g *gin.Context) {
		requestCtx := context.WithoutCancel(g.Request.Context())
		...
		var allMessages []dto.E2EMessage
		errMessageUnmarshal := json.Unmarshal(body, &allMessages)
		if errMessageUnmarshal != nil {
			logger.Error().Err(errMessageUnmarshal).Msg("failed to unmarshal request body into dto.E2EMessage")
			g.AbortWithStatus(http.StatusBadRequest)
			return
		...
		logger.Debug().Int("msgCount", len(allMessages)).Msg("sending messages")
		for _, messageIn := range allMessages {
			msgCtx := requestCtx
			if len(messageIn.TraceData) > 0 {
				msgCtx = monitoring.ExtractTraceData(msgCtx, messageIn.TraceData)
			}
			status, sendErrs = h.sendMessage(msgCtx, &messageIn)
			errs = errors.Join(errs, sendErrs)
		}
...
func (h *APIHandler) sendMessage(ctx context.Context, messageIn *dto.E2EMessage) (int, error) {
	logger := zerolog.Ctx(ctx).With().Str("testRunId", messageIn.TestRunId).Logger()
	...
	worker := func(id int) {
		...
		for keepOn {
			...
			case userId, ok := <-userIdQueue:
				...
				if sendErrors != nil {
					errs = errors.Join(errs, sendErrors)
				}
				wgWorkUnits.Done()
			case <-workerCtx.Done():
...
func (h *APIHandler) sendMessageToUserDevices(ctx context.Context, message *dto.E2EMessage, userId string) (int, error) {
	logger := zerolog.Ctx(ctx).With().Str("userId", userId).Logger()
